...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Restore saved settings and inputs
  restoreValues();
  // Wire inputs
  wireWater();
  wireStarter();
...
// Water Temp (°C)
// WaterTemp = DDT × N − (Room + Flour + Preferment + Friction)
function waterTempC(ddt, room, flour, prefermentEnabled, pref, method, customFriction) {
  const frictionMap = { hand: 1.0, planetary: 9.0, spiral: 11.0, custom: Number(customFriction) || 0 };
...
// Starter Rise (hours)
// inoc = seed / (seed + flour + water)
// hydration (%) = (water / flour) * 100
// t = k * inoc^(-α) * Q10^((Tref - T)/10) * flourFactor * hydrationFactor
...
// Bulk (hours)
// t = c * (starterPct/100)^(-β) * Q10^((26 - DDT)/10) * flourFactor * hydrationFactor * saltFactor
function bulkHours({ starterPct, ddtC, flourType, hydrBand, saltPct, c, beta, Q10 }) {
  ...
  const hours = c * Math.pow(sp / 100.0, -beta) * tempFactor * flourFactor * hydrationFactor * saltFactor;
  return hours;
}

/* ------------------ Wire & Compute: Water ------------------ */
function wireWater() {
  const ids = ['w_ddt','w_room','w_flour','w_pref_on','w_pref','w_method','w_custom'];
  ids.forEach(id => document.getElementById(id).addEventListener('input', computeWater));
...
function computeWater() {
  ...
  out.textContent = `${wt.toFixed(1)} °C`;
  note.textContent = '';
  note.classList.remove('warning');

  if (wt < 0) {
    note.textContent = 'Very cold water—use ice/chilled water to hit DDT.';
    note.classList.add('warning');
  } else if (wt > 60) {
    note.textContent = 'Very warm water—recheck inputs/friction.';
    note.classList.add('warning');
  }
  // Save
...
function restoreValues() {
  ...
  if (S) {
    setVal('set_st_k', S.st_k); setVal('set_st_a', S.st_a);
    setVal('set_q10', S.q10); setVal('set_st_tref', S.st_tref);
    setVal('set_b_c', S.b_c); setVal('set_b_b', S.b_b);
  }
  // Water
  const W = LS.get('water', null);
  if (W) {
    setVal('w_ddt', W.ddt); setVal('w_room', W.room); setVal('w_flour', W.flour);
    setChecked('w_pref_on', W.prefOn); setVal('w_pref', W.pref);
    setVal('w_method', W.method); setVal('w_custom', W.custom);
  ...
  if (St) {
    setVal('s_seed', St.seed); setVal('s_flour', St.flour); setVal('s_water', St.water);
    setVal('s_temp', St.tempC); setVal('s_flourtype', St.flourType);
  }
  // Bulk
  const B = LS.get('bulk', null);
  if (B) {
    setVal('b_sp', B.starterPct); setVal('b_ddt', B.ddtC); setVal('b_flourtype', B.flourType);
    setVal('b_hydr_band', B.hydrBand); setVal('b_salt', B.saltPct);
  }
}
